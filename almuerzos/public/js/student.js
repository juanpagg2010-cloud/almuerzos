...
function makeMenuCard(menu, confirmation) {
  const card = make(
    "article",
    undefined,
    "bg-white panel rounded-3xl overflow-hidden flex flex-col",
  );
  if (menu.imagenes?.[0]) {
  ...
  badges.append(
    make(
      "span",
      menu.diaSemana,
      "text-xs font-bold uppercase bg-brand-50 text-brand-700 px-3 py-1.5 rounded-full",
    ),
  );
  badges.append(
    make(
      "span",
      shortDate(menu.fecha),
      "text-xs font-semibold text-slate-400 py-1.5",
    ),
  );
  content.append(
    badges,
    make("h2", menu.platoPrincipal, "display text-2xl font-extrabold mt-5"),
  );
  if (menu.descripcion)
    content.append(make("p", menu.descripcion, "text-slate-500 mt-2 text-sm"));

  const details = make("div", undefined, "grid grid-cols-2 gap-3 mt-6 text-sm");
  details.append(
    make(
      "p",
      `Acompañamiento: ${menu.acompanamiento || "Por definir"}`,
  ...
  details.append(
    make(
      "p",
      `Bebida: ${menu.bebida || "Por definir"}`,
      "border-l-2 border-brand-200 pl-3",
    ),
  );
  ...
  if (confirmation) {
    response.append(
      make(
        "p",
        confirmation.asistira
          ? "✓ Confirmaste tu asistencia"
          : "Tu ausencia quedó registrada",
        "font-bold text-sm text-emerald-700",
      ),
    );
  } else {
    ...
    const yes = make(
      "button",
      "Sí, asistiré",
      "flex-1 bg-brand-600 text-white font-bold py-2.5 rounded-xl text-sm",
    );
    const no = make(
      "button",
      "No asistiré",
      "flex-1 bg-slate-100 text-slate-700 font-bold py-2.5 rounded-xl text-sm",
    );
...
async function loadMenus() {
  try {
    const [menusData, attendanceData] = await Promise.all([
      api("/menus"),
      api("/attendance/me"),
    ]);
    const menus = menusData.menus.sort(
      (a, b) => WEEKDAYS.indexOf(a.diaSemana) - WEEKDAYS.indexOf(b.diaSemana),
    );
    ...
    menus.forEach((menu) => {
      const confirmation = attendanceData.confirmations.find(
        (item) => item.menuId?._id === menu._id,
      );
...
async function confirmAttendance(menuId, asistira) {
  try {
    await api(`/attendance/menus/${menuId}`, {
      method: "PUT",
