...
async function loadOverview() {
  try {
    ...
    const today =
      menus.find(
        (menu) =>
          new Date(menu.fecha).toDateString() === new Date().toDateString(),
      ) || menus[0];
    ...
    attendance.confirmations.slice(0, 5).forEach((item) => {
      ...
      row.append(
        make("span", item.estudianteId.name, "font-semibold text-sm"),
        make(
          "span",
...
async function loadMenus() {
  try {
    ...
    menus.forEach((menu) => {
      ...
      const actions = make("div", undefined, "menu-actions");
      const edit = make(
        "button",
        "Editar",
      ...
      const remove = make(
        "button",
        "Eliminar",
        "rounded-xl px-3 py-2 bg-rose-50 text-rose-600 ml-2",
      );
      ...
      if (menu.estado === "Publicado") {
        const withdraw = make(
          "button",
          "Retirar de semana",
          "rounded-xl px-3 py-2 bg-amber-50 text-amber-700 mr-2",
        );
  ...
  try {
    ...
    data.users.forEach((account) => {
      ...
      row.append(
        ...
        make("td", shortDate(account.createdAt), "p-5 text-slate-500"),
      );
      body.append(row);
    });
  } catch (error) {
