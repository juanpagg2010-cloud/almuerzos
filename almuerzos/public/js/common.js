...
function userMessage(message, status) {
  ...
  const looksTechnical =
    !cleanMessage ||
    /\b(error|exception|stack|cast|mongo|mongoose|jwt|econn|enotfound|\bat\s+\w+\s*\()\b/i.test(
      cleanMessage,
    ) ||
    /[a-f0-9]{16,}|\d{8,}/i.test(cleanMessage);
  return looksTechnical
    ? GENERIC_ERRORS[status] ||
...
export async function api(path, options = {}) {
  ...
  try {
    ...
    throw new Error(
      "No fue posible conectarse. Revisa tu conexión e inténtalo de nuevo.",
    );
  }
  let data = {};
  try {
...
export function notify(message, error = false) {
  ...
  toast.className = `toast show rounded-2xl px-4 py-3 text-sm font-semibold shadow-xl text-white ${error ? "bg-rose-600" : "bg-slate-900"}`;
