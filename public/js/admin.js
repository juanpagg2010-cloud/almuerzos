...
getServerNow()
  .then((now) => {
    serverNow = now;
    $("#today-date").textContent = dateText(now);
  })
  .catch(() => {
    $("#today-date").textContent = dateText(serverNow);
  });
...
async function loadOverview() {
  try {
    ...
    const today =
      menus.find(
        (menu) => menuDateValue(menu.fecha) === serverDateValue(serverNow),
      ) || menus[0];
    const attendance = today
      ? await api(`/attendance/menus/${today._id}`)
      : { total: 0, asistiran: 0, noAsistiran: 0, confirmations: [] };
    ...
    attendance.confirmations.slice(0, 5).forEach((item) => {
      ...
      row.append(
        ...
        make(
          "span",
          item.asistira ? "Asiste" : "No asiste",
          "text-sm text-slate-500",
        ),
      );
      list.append(row);
    });
    if (!attendance.confirmations.length)
      list.append(
        make(
          "p",
...
async function loadMenus() {
  try {
    ...
    const summary = make(
      "p",
      `${publishedMenus.length}/5 menús publicados esta semana. Cada día solo puede tener uno.`,
      "text-sm text-slate-500 -mt-4 mb-2",
    );
    list.append(summary);
    menus.forEach((menu) => {
      const row = make(
        "article",
        undefined,
        "bg-white panel rounded-2xl p-5 flex flex-col md:flex-row gap-5 md:items-center",
      );
      const info = make("div", undefined, "flex-1");
      const visibleDay =
        menu.estado === "Publicado" ? ` · ${menu.diaSemana}` : "";
      info.append(
        make("h2", menu.platoPrincipal, "font-bold text-lg"),
        make(
          "p",
          `${shortDate(menu.fecha)} · ${menu.estado}${visibleDay}`,
          "text-sm text-slate-500 mt-1",
        ),
      );
      if (menu.imagenes?.length) {
        const pictures = make("div", undefined, "flex gap-2 mt-3");
        menu.imagenes.slice(0, 3).forEach((source) => {
      ...
      const edit = make(
        "button",
        "Editar",
        "rounded-xl px-3 py-2 bg-slate-100 text-slate-600",
      );
      edit.addEventListener("click", () => openMenuForm(menu));
      const remove = make(
        "button",
        "Eliminar",
        "rounded-xl px-3 py-2 bg-rose-50 text-rose-600",
      );
      ...
      if (menu.estado === "Publicado") {
        const withdraw = make(
          "button",
          "Retirar de semana",
          "rounded-xl px-3 py-2 bg-amber-50 text-amber-700",
        );
    ...
    if (!menus.length)
      list.append(
        make(
          ...
          "bg-white rounded-3xl p-12 text-center text-slate-500",
        ),
      );
  } catch (error) {
    notify(error.message, true);
...
function openMenuForm(menu) {
  $("#menu-form").reset();
  $("#menu-id").value = menu?._id || "";
  $("#form-title").textContent = menu ? "Editar menú" : "Nuevo menú";
  $("#menu-date-input").value = menu
    ? menuDateValue(menu.fecha)
    : serverDateValue(serverNow);
  $("#menu-weekday").value = menu?.diaSemana || "";
  if (menu) {
    $("#menu-main").value = menu.platoPrincipal;
    $("#menu-side").value = menu.acompanamiento || "";
    $("#menu-drink").value = menu.bebida || "";
    $("#menu-description").value = menu.descripcion || "";
  }
...
async function saveMenu(event) {
  ...
  try {
    ...
    const response = await api(`/menus${id ? `/${id}` : ""}`, {
    ...
    if (images.length) {
      ...
      await api(`/menus/${response.menu._id}/images`, {
        method: "POST",
        body: formData,
      });
    }
    notify(
      payload.estado === "Publicado"
        ? "El menú se publicó en la vista semanal."
        : "Menú guardado en la biblioteca.",
    );
    closeMenuForm();
    loadMenus();
  } catch (error) {
...
async function withdrawMenu(id) {
  try {
    ...
    notify(
      "El menú fue retirado de la vista semanal y sigue guardado en la biblioteca.",
    );
    loadMenus();
  } catch (error) {
    notify(error.message, true);
...
function openStudentForm(student) {
  ...
  $("#student-modal").classList.add("flex");
}

function closeStudentForm() {
  $("#student-modal").classList.add("hidden");
  $("#student-modal").classList.remove("flex");
}
...
function openAdminForm(admin) {
  ...
  $("#admin-modal").classList.add("flex");
}

function closeAdminForm() {
  $("#admin-modal").classList.add("hidden");
  $("#admin-modal").classList.remove("flex");
}

function groupLimit(grade) {
  return Number(grade) >= 10 ? 5 : Number(grade) >= 6 ? 6 : 8;
}
...
function syncStudentGroups(selectedGroup = "") {
  ...
  for (let group = 1; group <= limit; group += 1)
    input.append(
      new Option(
        `${group}`,
        group,
        false,
        String(group) === String(selectedGroup),
      ),
    );
}
...
async function saveAdmin(event) {
  ...
  try {
    await api(`/auth/admins/${id}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
    notify("Administrador actualizado correctamente.");
    closeAdminForm();
    loadUsers();
  } catch (error) {
    notify(error.message, true);
...
async function removeAdmin(admin) {
  if (
    !confirm(
      `¿Eliminar al administrador ${admin.name}? Esta acción no se puede deshacer.`,
    )
  )
    return;
  try {
    await api(`/auth/users/${admin._id}`, { method: "DELETE" });
...
async function removeUser(account) {
  if (
    !confirm(
      `¿Eliminar al usuario ${account.name}? Esta acción no se puede deshacer.`,
    )
  )
    return;
  try {
    await api(`/auth/users/${account._id}`, { method: "DELETE" });
    notify("Usuario eliminado correctamente.");
    loadUsers();
  } catch (error) {
    notify(error.message, true);
...
function toggleStudentFields() {
  ...
  $("#user-grade").required = isStudent;
  $("#user-group").required = isStudent;
...
async function loadUsers() {
  try {
    ...
    data.users.forEach((account) => {
      ...
      row.append(
        make("td", account.name, "p-5"),
        make("td", account.email, "p-5 text-slate-500"),
        make(
          "td",
          account.role === "Admin" ? "Administrador" : "Estudiante",
          "p-5",
        ),
        make(
          "td",
          account.role === "Estudiante"
            ? `${account.grado} / ${account.grupo}`
            : "—",
          "p-5",
        ),
        make("td", dateTimeText(account.createdAt), "p-5 text-slate-500"),
      );
      ...
      if (account.role === "Estudiante") {
        const edit = make(
          "button",
          "Editar",
          "rounded-lg bg-slate-100 px-3 py-2 text-sm font-bold text-brand-700",
        );
        edit.addEventListener("click", () => openStudentForm(account));
        const remove = make(
          "button",
          "Eliminar",
          "rounded-lg bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700",
        );
        remove.addEventListener("click", () => removeUser(account));
