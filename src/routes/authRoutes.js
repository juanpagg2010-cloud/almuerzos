...
router.get("/students", protect, authorizeRoles("Admin"), async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message: error.message || "No se pudo consultar el registro.",
      });
  }
});
...
router.get("/users", protect, authorizeRoles("Admin"), async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message: error.message || "No se pudo consultar los usuarios.",
      });
  }
});
...
router.delete(
  ...
  async (req, res) => {
    try {
      await deleteUserByAdmin(req.params.id, getUserId(req.user));
      return res
        .status(200)
  ...
  async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          ok: false,
          message: error.message || "No se pudo actualizar el estudiante.",
        });
    }
  },
);
...
router.delete(
  ...
  (req, res) =>
    res
      .status(405)
      .json({ ok: false, message: "Los estudiantes no se pueden eliminar." }),
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      const user = await updateOtherAdmin(
        req.params.id,
        getUserId(req.user),
        req.body,
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ ok: true, message: "Administrador eliminado correctamente." });
    } catch (error) {
      return res
        .status(error.statusCode || 400)
        .json({
          ok: false,
          message: error.message || "No se pudo eliminar el administrador.",
        });
    }
  },
);
